...
$query = "SELECT C.service_label, (SUM(B.value) - SUM(A.value)) as trade_surplus_value
FROM task2.gsquarterlySeptember20_1 A, task2.gsquarterlySeptember20_1 B, task2.services_classification C
WHERE 
A.country_code = B.country_code AND 
A.code = C.code AND
A.account = 'Imports' AND
B.account = 'Exports' AND
A.product_type = 'Services' AND
B.product_type = 'Services' AND
A.time_ref IN (select time_ref
from (SELECT time_ref, SUM(value) as Trade_value
FROM task2.gsquarterlySeptember20_1
GROUP BY time_ref 
ORDER BY SUM(value) DESC 
LIMIT 10)) AND 
A.country_code IN (select country_code
from(SELECT C.country_code, A.product_type, A.status, (SUM(A.value) - SUM(B.value)) as trade_deficit_value
...
A.STATUS = 'F' AND
B.STATUS = 'F' 
GROUP By C.country_code, A.product_type, A.status 
ORDER By trade_deficit_value DESC
LIMIT 50))
GROUP By C.service_label, A.product_type
